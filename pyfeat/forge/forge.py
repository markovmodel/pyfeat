...
class Forge(pt.TRAMData):
    ...
    def __init__(self,
                 ...
                 kT_target=None,
                 verbose=False):
        r"""Forge that prepares data needed for all estimators

        Parameters
        ----------

        traj : list of dictionaries 
            contianing the trajectory inforamtion
        b_K_i : np.array
            Biases
        kT_K : np.array
            array of kT values
        kT_target : int
            target value at which the probability and free energy should be 
            estimated
        verbose : boolean
            prints usefull information to stdout
        """
        super(Forge, self).__init__(trajs,
                                    b_K_i=b_K_i,
                                    kT_K=kT_K,
                                    kT_target=kT_target,
                                    verbose=verbose)
    ...
    def M_K_x(self):
        if self._M_K_x is None:
            ...
            self._M_K_x = np.zeros(shape=(self.n_therm_states, self.N_K.max()),
                                   dtype=np.intc)
    ...
    def b_IK_x(self):
        if self._b_IK_x is None:
            ...
            self._b_IK_x = np.zeros(shape=(self.n_therm_states,
                                           self.n_therm_states,
                                           self.N_K.max()), dtype=np.intc)
        ...
        for traj in self.trajs:
            ...
            for t in xrange(traj['t'].shape[0]):
                ...
                for I in xrange(self.n_therm_states):
                    self._b_IK_x[I, K, t_K[K]] = self.kT_K[K] * (1.0 / self.kT_K[I]
                                                                 - 1.0 / self.kT_K[self.kT_target]) * traj['b'][t]
                t_K[K] += 1
        ...
        for traj in self.trajs:
            if traj['u'].shape[1] == 1:
                raise pt.ExpressionError(
                    "b_IK_x",
                    "Trajectory with single energy columns detected -"
                    "use kT file and kT target"
                )
            if traj['u'].shape[1] != self.n_therm_states:
                raise pt.ExpressionError(
                    "b_IK_x",
                    "Trajectory with wrong number of energy columns "
                    "detected (%d!=%d)" \
